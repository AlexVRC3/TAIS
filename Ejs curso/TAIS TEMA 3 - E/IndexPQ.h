...
template <typename Q = string, typename T = int, typename Comparator = std::less<T>>
class IndexPQ {
public:
...
private:
    ...
    std::unordered_map<Q, int> posiciones;   // un 0 indica que el elemento no está
...
public:
    ...
    void update(string e, T const& p) {
    ...
    void update2(Q e, T& p) {
        ...
        if (!posiciones.count(e)) {
            ...
            int i = posiciones.at(e);
            array[i].prioridad.first = array[i].prioridad.first + p.first;
