...
/*@ <answer>
    ...
    formas(k,s) y la solución quedará almacenada en la ultima posicion de la matriz. Esta es la version no optimizada ya que se puede reducir el coste en espacio adicional utilizando
    ...
    El coste en espacio es O(k*s) debido al uso de la matriz para almacenar los resultados de los subproblemas.
    ...
    for (int i = 1; i <= k; i++)
    {
        ...
        for (int j = 0; j <= s ; j++)
        {
            if (i > j) {
                ...
                formas[i][j] = formas[i - 1][j] + formas[i][j - i];
            }
        }
    }
    return formas[k][s];
